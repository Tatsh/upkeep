...
def test_portage_env(sp_mocker: SubprocessMocker, mocker: MockerFixture) -> None:
...
def test_portage_env_error_falls_back(error: Exception, mocker: MockerFixture) -> None:
...
def test_portage_env_unknown_variable(mocker: MockerFixture) -> None:
...
def test_build_directory_follows_portage_tmpdir(mocker: MockerFixture) -> None:
    mocker.patch('upkeep.utils.portage.portage_env', return_value='/home')
    assert build_directory() == Path('/home/portage')


def test_binary_package_directory(mocker: MockerFixture) -> None:
