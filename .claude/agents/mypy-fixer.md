...
Fixes mypy type errors and eliminates `Any` usage in the upkeep project.
...
1. Fix all mypy errors reported by `uv run mypy upkeep`
...
Place new TypedDict classes in `upkeep/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`upkeep/typing.py` when shared.
...
1. Run `uv run mypy upkeep` and capture all errors.
...
4. Run `uv run mypy upkeep` again to verify fixes.
