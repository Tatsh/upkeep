...
If any changed files are under `upkeep/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `upkeep/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `upkeep/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `upkeep/commands/main.py`,
...
For Python files, strip the `upkeep/` prefix and replace `/` with `.` (like module imports).

- Python file `upkeep/media.py` → `media:`.
- Multiple files under `upkeep/commands/` → `commands:`.
- Single command file `upkeep/commands/admin.py` → `commands.admin:`.
